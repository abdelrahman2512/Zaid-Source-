...
async def ban_globally(_, message):  
    if not message.reply_to_message:
        ...
        if user.id == from_user.id:
            ...
            m = await message.reply_text(f"✶ **المستخدم:  {user.mention}**\n✶ تم حظره عام بنجاح")    
            number_of_chats = 0
            for sex in served_chats:
                try:
                    ...
                    pass    
            ban_text = f"""
✶ **عضو جديد على قائمة المحظورين عام**
...
**Chats:** {number_of_chats}"""
    ...
    if user_id == from_user_id:
        ...
        if is_gbanned:
            await message.reply_text("✅ **تم حظر المستخذم بنجاح.**")
        else:
            ...
            for chat in chats:
                served_chats.append(int(chat["chat_id"]))
            m = await message.reply_text(f"✶ **المستخدم {mention}**\n✶ تم حظره عام بنجاح")    
            number_of_chats = 0
            for sex in served_chats:
                try:
...
async def chat_watcher_func(_, message):
    ...
    if await is_gbanned_user(userid):
        try:
            ...
            return       
        await message.reply_text(f"{checking} ✶ هذا الشخص ضمن قائمة المحظورين عام لذلك قمت بحظره في المجموعة")

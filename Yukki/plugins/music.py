...
@Client.on_message(filters.command(["تشغيل", "/play"], prefixes=f""))
async def play(_, message: Message):
    await message.delete()
    chat_id = message.chat.id
    if not await is_served_chat(chat_id):
        await add_served_chat(chat_id)
        await message.reply(f"- تم تفعيل المجموعة بنجاح يمكنك استخدام البوت الآن. ", disable_web_page_preview=True)
        return
...
def song(_, message):
    query = " ".join(message.command[1:])
    print(query)
    m = message.reply("**✶ جار البحث انتظر قليلاً ..**")
    ...
    try:
        ...
        m.edit("**✶ لم يتم العثور على نتائج\n✶ اكتب المطلوب بشكل صحيح. **")
        print(str(e))
        return
    ...
    try:
        ...
        buttons = [[InlineKeyboardButton("✯", url="t.me/MDDDJ")]]
        reply_markup = InlineKeyboardMarkup(buttons)
...
@Client.on_message(filters.command("تحميل فيديو", [".", ""]))

...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            await message.reply_text(f"**- العضو {user.mention} \n- تم رفعه مطور**")
    ...
    if added:
        await message.reply_text(f"**- العضو {mention} \n- تم رفعه مطور**")
        return os.execvp("python3", ["python3", "-m", "Yukki"])
    await edit_or_reply(message, text="something wrong happened, check logs.")  
    return    
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            await message.reply_text(f"- المستخدم **{user.mention}**\n- تم حذفه من قائمة المطورين")
            return os.execvp("python3", ["python3", "-m", "Yukki"])

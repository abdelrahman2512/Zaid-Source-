...
async def broadcast_message_pin(_, message):
    if not message.reply_to_message:
        ...
        msg = await message.reply_text("✶ جار الاذاعه بالتثبيت")
        ...
        for i in chats:
            try:
                ...
                pass
        await msg.edit_text(f"تمت الاذاعه في {sent} مجموعة\nتم التثبيت في {pins} دردشة.")  
        return
...
async def broadcast_message_nopin(_, message):
    if not message.reply_to_message:
        ...
        msg = await message.reply_text("✶ جار الاذاعه")
        x = message.reply_to_message.message_id
        y = message.chat.id
        sent = 0
        chats = []
